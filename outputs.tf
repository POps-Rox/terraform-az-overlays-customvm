...
output "linux_vm_id" {
  ...
  sensitive   = true
  value       = local.custom_boot_image_os_type == "linux" ? azurerm_virtual_machine.custom_vm.*.id : null
}

output "windows_vm_id" {
  description = "Id for the Linux VM, if multiple VM's are created then it will be a list of ids"
  sensitive   = true
  value       = local.custom_boot_image_os_type == "windows" ? azurerm_virtual_machine.custom_vm.*.id : null
}

output "linux_vm_name" {
  description = "Name for the Linux VM, if multiple VM's are created then it will be a list of names"
  sensitive   = true
  value       = local.custom_boot_image_os_type == "linux" ? azurerm_virtual_machine.custom_vm.*.name : null
}

output "windows_vm_name" {
  description = "Name for the Linux VM, if multiple VM's are created then it will be a list of names"
  sensitive   = true
  value       = local.custom_boot_image_os_type == "windows" ? azurerm_virtual_machine.custom_vm.*.name : null
}

output "admin_ssh_key_public" {
  description = "The generated public key data in PEM format"
  value       = var.disable_password_authentication == true && var.generate_admin_ssh_key == true && local.custom_boot_image_os_type == "linux" ? tls_private_key.rsa[0].public_key_openssh : null
}

output "admin_ssh_key_private" {
  description = "The generated private key data in PEM format"
  sensitive   = true
  value       = var.disable_password_authentication == true && var.generate_admin_ssh_key == true && local.custom_boot_image_os_type == "linux" ? tls_private_key.rsa[0].private_key_pem : null
}
...
output "windows_vm_public_ips" {
  ...
  value       = var.enable_public_ip_address == true && local.custom_boot_image_os_type == "windows" ? zipmap(azurerm_virtual_machine.custom_vm.*.name, azurerm_public_ip.pip.*.ip_address) : null
}


output "linux_vm_public_ips" {
  description = "Public IP's map for the all windows Virtual Machines"
  value       = var.enable_public_ip_address == true && local.custom_boot_image_os_type == "linux" ? zipmap(azurerm_virtual_machine.custom_vm.*.name, azurerm_public_ip.pip.*.ip_address) : null
}


output "linux_virtual_machine_ids" {
  description = "The resource id's of all Linux Virtual Machine."
  value       = local.custom_boot_image_os_type == "linux" ? concat(azurerm_virtual_machine.custom_vm.*.id, [""]) : null
}

output "windows_virtual_machine_ids" {
  description = "The resource id's of all Windows Virtual Machine."
  value       = local.custom_boot_image_os_type == "windows" ? concat(azurerm_virtual_machine.custom_vm.*.id, [""]) : null
}
...
output "linux_vm_identity" {
  ...
  value       = local.custom_boot_image_os_type == "linux" ? azurerm_virtual_machine.custom_vm.*.identity : null
}

output "windows_vm_identity" {
  description = "Windows Identity block with principal ID"
  value       = local.custom_boot_image_os_type == "windows" ? azurerm_virtual_machine.custom_vm.*.identity : null
}
